...
* **単語の自動生成と補完 (API連携)**: 「英単語」を入力するだけで、以下のデータを自動で取得・生成します。単語帳に不足しているデータの一括補完機能も備えています。
  * 英語の定義・品詞・発音記号（WordsAPI / cmu-dict）
  * 高精度な日本語訳（DeepL API）
  * TOEIC（ビジネス・旅行・フォーマル等）に特化した文脈での短い例文と、自然な穴埋め問題化（WordsAPI / OpenAI gpt-4o-mini）
* **プロジェクト管理画面**: 各単語の正答率、連続正解数、殿堂入りステータスの確認、個別編集、手動追加が一つのダッシュボードで完結します。不足データの「一括補完（Batch Enrich）」もここから実行可能です。
* **データのエクスポート/インポート**: 学習進捗を含めたプロジェクト全体のZIPバックアップや、プレーンな単語リストとしてのCSV出力（エクセル対応BOM付き）が可能です。
