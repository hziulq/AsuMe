...
- **一部データの補完生成 (Enrich)**: 問題の手動追加時や個別修正時に、英単語・意味・品詞から「例文と発音記号のみ」をワンボタンで自動生成する機能 (`/api/enrich`) を実装。
- **不足データの一括補完 (Batch Enrich)**: 日本語訳や例文が不足しているCSVを取り込んだ際、管理画面から一括で自動生成・補完する機能を実装。
- **エクスポートの拡張**: バックアップ用のZIPに加え、プレーンな単語データとしてCSV形式でも出力（ダウンロード）できる機能を実装。
